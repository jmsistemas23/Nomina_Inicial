(function($){
	...
	function init(target, filters){
		...
		initCss();
		createFilter(true);
		var filtroscolumnas = sessionStorage.getItem('filtroscolumnas');
		if (filtroscolumnas != "undefined") {
		    createFilter();
		}
		if (opts.fitColumns){
			setTimeout(function(){
		...
		function createFilter(frozen){
			...
			for(var i=0; i<fields.length; i++){
				...
				//var filtroscolumnas = sessionStorage.getItem('filtroscolumnas').split('|');;
				//var index = $.inArray(field, filtroscolumnas);
