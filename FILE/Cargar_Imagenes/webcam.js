...
$(document).ready(function () {
    ...
    $('#btnLimpiar').bind('click', function () {
        $('#txtempleado').numberbox('setValue', '');
    ...
    texts.numberbox('textbox').bind('keydown', function (e) {
    ...
    $('#btnFoto').bind('click', function () { configure(); });

    $('#btnDetener').bind('click', function () { Detener('#btnDetener'); });

    $('#btnGFoto').bind('click', function () { Guardar_Foto('#btnGFoto'); });

    $('#btnLFoto').bind('click', function () {LIMPIAR_FOTO('#btnLFoto'); });
...
function LIMPIAR_DATOS()
{
    ...
    $('#btnGFoto').linkbutton({ disabled: true });
    $('#btnLFoto').linkbutton({ disabled: true });
        
    $('#btnLFirma').linkbutton({ disabled: true });
    ...
    $(dgcontrol).datagrid({
        ...
        onClickRow: function () {
            ...
            if (rows) {
                ...
                localStorage.setItem('emp',rows.numemp);
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.d[0] == "1") {
                ...
                document.getElementById('dimg').innerHTML = '<img id="imageprev" src="data:image/jpeg;base64,' + imagen + '"/>';
                $('#btnLFoto').linkbutton({ disabled: false });
...
// configurar para abrir video de camara       
function configure() {
    var texts = $('#txtempleado').numberbox('getValue');
    if (texts != "") {
        $('#my_camera').show();
        $('#dimg').hide();       
        Webcam.set({
            ...
            height: 200,
            image_format: 'jpeg',
            jpeg_quality: 90, swffile: '/webcam.swf',
        });
    ...
    else { $.messager.alert('Error', 'Falta Buscar el Empleado', 'error'); }
}

//congelar video para sacar imagen
function Detener(btnobj) {
    // play sound effect
    //shutter.play();
    if ($(btnobj).linkbutton('options').disabled) { return false; }
    else
    {
        ...
        Webcam.reset();
        $('#btnGFoto').linkbutton({ disabled: false });
    }
}

//grabar imagen    	   
function Guardar_Foto(btnobj) {
    if ($(btnobj).linkbutton('options').disabled) { return false; }
    else
    {
        ...
        var parametros = {};
        parametros.imageData = base64image;
        parametros.empleado = localStorage.getItem('emp');
        parametros.tipo = 'Foto';
        $.ajax({
            type: 'POST',
            async: true,
