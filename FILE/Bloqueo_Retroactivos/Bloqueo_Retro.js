...
$(document).ready(function () {
    ...
    $('#btnMP').bind('click', function () {        
        LISTAR_DOCUMENTOS("");
        windows("#wdoc", "100%", 510, false, "Movimientos");
        var text = $('#txtvalor');
    ...
    $('#btnLDoc').bind('click', function () {       
        ...
        $('#cbocondicion').combobox('setValue', '=');
        $('#cbocampos').combobox('setValue', 'numdocmp');        
        LISTAR_DOCUMENTOS("");
    });
...
function onCheck(index, row) {
    for (var i = 0; i < chkRows.length; i++) {
        if (chkRows[i].numdocmp == row.numdocmp) {
            return
        }
    }
    chkRows.push(row);
...
function CARGAR_DOCUMENTOS_SELECCIONADOS(dgcontrol) {
    ...
    for (var i = 0; i < chkRowsDoc.length; i++) {
            $(dgcontrol).datagrid('insertRow', {
                ...
                row: {
                    id: i,
                    numdocmp: chkRowsDoc[i].numdocmp,
                    cvemovmp: chkRowsDoc[i].cvemovmp,
                    desmovmp: chkRowsDoc[i].desmovmp,
                    numplamp: chkRowsDoc[i].numplamp,
                    numempmp: chkRowsDoc[i].numempmp,
                    numpladmp: chkRowsDoc[i].numpladmp,
                    numemdmp: chkRowsDoc[i].numemdmp,
                    rfcempmp: chkRowsDoc[i].rfcempmp,
                    nomcommp: chkRowsDoc[i].nomcommp,
                }
            });
        $(dgcontrol).datagrid('beginEdit', i);
        var varchar = $(dgcontrol).datagrid('getEditor', { index: i, field: 'tipoplaza' });
        $(varchar.target).combobox('setValue', 'D');
    }   
    chkRowsDoc = [];  
...
function GUARDAR_DOCUMENTOS(btnobj) {
    if ($(btnobj).linkbutton('options').disabled) { return false; }
        ...
        if (chkRows.length == 0) { $.messager.alert('Error', 'Falta Seleccionar los Documentos', 'error'); }
        else {
            for (var j = 0; j < chkRows.length; j++) {
                if (chkRows[j].numdocmp != "") {
                    cadena += chkRows[j].numdocmp + ":" + chkRows[j].tipoplaza+ "|";
                }
                else { cadena = ""; break; }
            }
