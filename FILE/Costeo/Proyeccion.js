...
$(document).ready(function () {
    ...
    $('#dgfiltro').datagrid({      
        ...
        if (rows.sexo == 'M')
            ...
            if (rows.titulacion == '1') { document.getElementById('rbTitulasi').checked = true; }
            if (rows.titulacion == '0') { document.getElementById('tbTitulano').checked = true; }
...
function MostrarNivelSalarial(btnobj,filtro) {
    if ($(btnobj).linkbutton('options').disabled) { return false; }
    else
    {      
            var parametros = {};
            ...
            $.ajax({
                type: "POST",
                url: 'funsiones.aspx/columnas_nivel',
                dataType: "json",
                ...
                success: function (data) {
                    ...
                    var objcol = jQuery.parseJSON(data.d[1]);

                    var $datagrid = {};
                    var columns = new Array();
                    for (var col = 0; col < objcol.length; col++) {
                        columns.push({ "field": objcol[col].name, "title": objcol[col].name, "align": 'center', "halign": 'center' });
                    }
                    $datagrid.columns = new Array(columns);
                    $('#tblBusquedaDeForSubNivel').datagrid({ columns: "", url: "" });
                    $('#tblBusquedaDeForSubNivel').datagrid($datagrid);                   
                },
                error: function (err) {
                    $('#loading').hide(100);
...
function listarNivelSalarial(filtro) {
    ...
    {       
        ...
        $('#tblBusquedaDeForSubNivel').datagrid({
            //url: 'listarForSubNivel.aspx?busqueda=' + filtro + '&nivel=' + codnivel + '&columnas=' + strcol,
            url: 'listarForSubNivel.aspx?busqueda=' + filtro + '&nivel=' + codnivel,
            pagination: true,
            enableFilter: true,
            rownumbers: true,
            singleSelect: true,
...
function AGREGAR_VALORES_FILTRO() {      
    ...
    var quinquenio = "", hijos = "", sexo = "", tiporegimen = "", porcregimen = "",volregimen="",conceptos="",titulacion="";
                        ...
                        if ($('#txtcvepuesto').textbox('getValue') == "") { $.messager.alert('Error', 'Falta el puesto', 'error'); return 0; }
                            ...
                            if ($('#txtnivel').textbox('getValue') == "") { $.messager.alert('Error', 'Falta el nivel salarial', 'error'); return 0; }
                                ...
                                if (($('#cboTipoRegimen').combobox('getValue') == 'T') && ($('#cboPorcRegimen').combobox('getValue') == "x")) { $.messager.alert('Error', "Falta seleccionar el porcentage del regimen", 'error'); return 0; }                              
                                    ...
                                    if ($('#rbTitulano').is(":checked") == true) { titulacion = '0'; }
                                    if ($('#rbTitulasi').is(":checked") == true) { titulacion = '1'; }
    ...
    else
    {
            ...
            for (var i = 0; i < dg.datagrid('getData').total; i++) {
                ...
                cadena += 'cantidad:' + dg.datagrid('getRows')[i].numplaza + "|cveadspl:" + dg.datagrid('getRows')[i].uniresp + "|cvepuepl:" + dg.datagrid('getRows')[i].puesto +
                    "|cvenivpl:" + dg.datagrid('getRows')[i].cvenivpl + "|quinque:" + dg.datagrid('getRows')[i].quinquenio + "|titulacion:" + dg.datagrid('getRows')[i].titulacion +"|sexemp:" + dg.datagrid('getRows')[i].sexo + "|padomad:" + dg.datagrid('getRows')[i].hijos +
            ...
            IR_PAGINA("Consulta_Grid_kendo.aspx", "tipo=Proyeccion&filtro=" + cadena.substring(0, cadena.length - 1) + "&conceptos=&canceladas=0" + "&btntipo=");
