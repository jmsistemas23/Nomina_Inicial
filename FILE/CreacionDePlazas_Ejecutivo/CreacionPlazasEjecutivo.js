...
$(document).ready(function () {
    ...
    HABILITAR_DESHABILITAR_MENU();

    $('#aNormal').bind('click', function () { PLAZA_NORMAL('NM','PLAZA NORMAL'); });
    $('#aInterinas').bind('click', function () { tipoPlazaSeleccionada = "IN"; mostrarBuscadorDePlazas('PLAZAS INTERINAS'); });
    //$('#aHonorarios').bind('click', function () { PLAZA_NORMAL('H', 'PLAZAS DE HONORARIOS'); });
    $('#aJubilados').bind('click', function () { PLAZA_NORMAL('JP', 'PLAZAS DE JUBILADOS Y PENSIONADOS'); });
    $('#aNoEmpleados').bind('click', function () { PLAZA_NORMAL('NE', 'PLAZAS DE NO EMPLEADOS'); });
    ...
    var txtcvenivsal = $('#txtnivel');
    txtcvenivsal.textbox('textbox').bind('keydown', function (e) {
        if (e.keyCode == 13) {
            listarForSubNivel(txtcvenivsal.val());
    ...
    //txtcvepagad.textbox('textbox').bind('blur', function (e) {
    //    listarPagaduria(txtcvepagad.val());
    //});
...
function HABILITAR_DESHABILITAR_MENU()
{ 
    $.ajax({
        type: "POST",
        url: 'UtileriasBD.aspx/Listar_CreacionPlazas', 
        dataType: "json",
        ...
        success: function (data) {
            ...
            if (parseInt(data.d[1]) > 0) {
                $('#dbotones').show();
                $('#lblmensaje').hide();
                if (obj[0].PlaNormales == "Si")
                { $('#aNormal').show(); }
                if (obj[0].PlaInterinos == "Si")
                { $('#aInterinas').show(); }
                if (obj[0].PlaJubPen == "Si")
                { $('#aJubilados').show(); }
                if (obj[0].PlaNoEmp == "Si")
                { $('#aNoEmpleados').show(); }
            }
            else { $('#lblmensaje').show(); }
        },
        error: function (err) {
            $('#loading').hide(100);
            $.messager.alert('Error', err.responseJSON.Message, 'error');
        },
        complete: function ()
        { $('#loading').hide(100); }
    });
...
function PLAZA_NORMAL(tipo, titulo) {
    ...
    $('#btnBuscar').hide();   
            ...
            Cargar_Combo('#cbovacancia', "UtileriasBD.aspx/Lista_TipoVacantes", tipoPlazaSeleccionada);
}
...
function mostrarBuscadorDePuesto() {
    if ($('#txtdespagad').textbox("getValue") == '') { $.messager.alert('Error', "Debe especificar la pagaduria", 'error'); return 0; }
    else
    {
...
function listarPuestos(filtro) {
        ...
        if (!$('#divTableDatosDePuesto').is(':hidden'))
            $('#divTableDatosDePuesto').toggle(1000);
        ...
        $('#tblBusquedaDePuestos').datagrid({
            url: 'listarPuestos.aspx?busqueda=' + filtro + '&tipoplaza=' + tipoPlazaSeleccionada,
            pagination: true,
            enableFilter: true,
            rownumbers: true,
            singleSelect: true,
...
function listarCentroCosto(filtro) {
    ...
    $('#tblBusquedaDeCentroCosto').datagrid({
            ...
            onClickRow: function () {
                ...
                //TIPOPLAZA('#cboTipoPlaza', "UtileriasBD.aspx/Lista_TipoPlazas", row.Clave);
            }
        });  
}
...
function Cargar_Combo(objddl, url, valor) {
    var parametros = {};
...
function TIPOPLAZA(objddl, url,cvecentro) {
    var parametros = {};
...
function crearPlaza() {
    ...
    parametros.estatus = "VD";//$('#cbovacancia').combobox("getValue");
    parametros.horas = 0;//$('#txthoras').textbox("getValue");
