<%@ Page Language="C#" AutoEventWireup="true" %>

<%        
    string pagina =(Request.Form["page"] == null)?"1": Request.Form["page"].ToString();
    string rows = (Request.Form["rows"] == null) ? "20" : Request.Form["rows"].ToString();
    string busqueda = Request.QueryString["busqueda"].ToString();
    string clave = Request.QueryString["clave"].ToString();
    int paginasN = Convert.ToInt32(pagina);
    int rowsN = Convert.ToInt32(rows);
    
    Utilerias lib = new Utilerias();

    //System.Data.DataSet ds = lib.ejecutarConsultaEnDataSet("GESRH_SPT_CalculoPerfiles_ListarProcedimientosAsignados @clave='" + clave + "',@desde='" + ((paginasN * rowsN) - rowsN + 1).ToString() + "', @hasta='" + (paginasN * rowsN).ToString() + "', @valor='" + busqueda + "'");
    System.Data.DataSet ds = lib.ejecutarConsultaEnDataSet("GESRH_SPT_CalculoPerfiles_ListarProcedimientosAsignados @clave='" + clave + "', @valor='" + busqueda + "'");

    //string totRegs = ds.Tables[1].Rows[0][0].ToString();
    
    //string resultado = "{\"rows\":" + new Utilerias().convertirDatatableEnJsonString(ds.Tables[0]) + ",\"total\":\"" + totRegs + "\"}";

    string resultado = "{\"rows\":" + new Utilerias().convertirDatatableEnJsonString(ds.Tables[0]) + "}";

    Response.Write(resultado);
 %>
