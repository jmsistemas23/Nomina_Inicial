...
<%        
    ...
    string procs = Request.QueryString["procs"].ToString();
    int paginasN = Convert.ToInt32(pagina);
    int rowsN = Convert.ToInt32(rows);
    
    Utilerias lib = new Utilerias();

    //System.Data.DataSet ds = lib.ejecutarConsultaEnDataSet("GESRH_SPT_CalculoPerfiles_ListaProcedimientoParaAsignacion @clave='" + clave + "',@desde='" + ((paginasN * rowsN) - rowsN + 1).ToString() + "', @hasta='" + (paginasN * rowsN).ToString() + "', @procs='" + procs + "'");
    System.Data.DataSet ds = lib.ejecutarConsultaEnDataSet("GESRH_SPT_CalculoPerfiles_ListaProcedimientoParaAsignacion @clave='" + clave + "', @procs='" + procs + "'");
