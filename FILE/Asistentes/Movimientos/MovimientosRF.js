...
function GUARDAR() {
    var valor = "";
    if ($('#cboInd').combobox('getValue') != 'x') { valor = $('#cboInd').combobox('getValue'); }
    else { $.messager.alert('Error', "Falta indice del movimiento", 'error'); }

    if ($('#txtnombremenu').textbox('getValue') == "") { $.messager.alert('Error', "Falta nombre del movimiento", 'error'); }
    else
    {                
        var qry = $('#txtId').val() + '|' + $('#txtnombremenu').val().toUpperCase() + '|' + $('#txtpropietario').val() + '|' + ((document.getElementById('chkvisible').checked) ? '1' : '0') + '|' + valor;
        var parametros = {};
        parametros.tipo = tipo;
