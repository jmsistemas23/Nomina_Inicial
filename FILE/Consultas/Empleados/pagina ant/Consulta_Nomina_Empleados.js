    ...
    $.ajax({
        ...
        complete: function ()
        {
            if (obj.length > 0) {
                ...
                $('#txtpagaduria').textbox('setValue', obj[0].cvepagad + "-" + obj[0].descpagad);
                ...
                $('#txttippago').textbox('setValue', obj[0].tipopago+ "-" +obj[0].despago);
                $('#txtbanco').textbox('setValue', obj[0].cvebanor+ "-" +obj[0].banco);
...
function CARGAR_DATOS(dgcontrol, ancho, alto, strnomemp) {
    var rows
    $(dgcontrol).datagrid({
        url: "Listar_Plazas.aspx?busqueda=" + strnomemp,
        pagination: true,
        rownumbers: true,
        singleSelect: true,
        striped: true,       
