...
function CARGAR_DOC(dgcontrol, ancho, alto) {
    ...
    $(dgcontrol).datagrid({
        url: 'Listar_Pensionadas.aspx?busqueda=' + con,
        pagination: true,      
        rownumbers: true,
        singleSelect: true,
        striped: true,
        ...
        onClickRow: function () {
            ...
            if (rows) {
                var mov = rows[fields[6]];
                sessionStorage.setItem('cvedoc', mov);               
                MOSTRAR_CONSULTA();                
            }
        },
        error: function (er) {
        ...
        success: function (data) {
            ...
            else
            {
                ...
                if (objcamposOrigen.length > 0) {
                    origencreada = true;                   
                    CREAR_CAPTURA(objcamposOrigen, objdatos, '#ddatos', 'tblo');
                }
